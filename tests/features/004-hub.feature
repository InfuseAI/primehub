...
Feature: Hub
  ...
  Scenario: User can start/stop the JupyterLab server with GPU
    ...
    Then I should see text of element with xpath "//div[@style='position: relative;']//span[@role='presentation']" is matched the regular expression "NVIDIA-SMI\s+\d+\W\d+\W\d+\s+Driver Version:\s+\d+\W\d+\W\d+\s+CUDA Version:\s+\d+\W\d+"
